...
def _downscale_mode(arr: NDArray[np.int8], factor: int) -> NDArray[np.int8]:
    ...
    Passable transparent pixels (-1) are ignored unless they are the majority.
    Invisible solid pixels (-2) remain candidates so their collision behavior
    ...
    for i, blk in enumerate(blocks):
        solid = blk[blk != -1]
        passable = blk[blk == -1]
        if passable.size > solid.size:
            out[i] = -1
        else:
            values, counts = np.unique(solid, return_counts=True)
            max_count = counts.max()
            # np.unique sorts values. Taking the last tied value preserves the
            # original "highest palette index wins" behavior.
            out[i] = values[counts == max_count][-1]
...
class Sprite:
    ...
    def to_ascii(self, rendered: bool = False) -> str:
        ...
        Args:
            rendered: If true, include the current rotation, mirrors, and scale.
    ...
    def color_remap(self, old_color: ColorLike | None, new_color: ColorLike) -> "Sprite":
        ...
        Args:
            old_color: The old color symbol to remap, or None to remap all
                visible colors.
    ...
    def merge(self, other: "Sprite") -> "Sprite":
        ...
        Only -1 cells are holes. Visible colors and invisible-solid -2 cells
        participate, and this sprite takes precedence where both have cells.
